		...
		instance.animations.each( function(id, anim){ //Each animation registered
			if( anim.state == 'play'){ // only the ones playing
				...
				instance.models[ anim.model ].each(function(bone_id, bone){ // Each bone
					...
					should_continue = bone.each(function(pos, state){ // must find the current state to draw
						var i = parseInt( pos );
	...
	function register(id, model, cb ){
		...
		if( instance.models[id].length < 1 ){ //FAIL
		  ...
		  var arr = [];
			...
			instance.models[id].each(function(i,e) {
				arr.push( e[0].img_url );
			});
			instance.img_manager.mass_register(arr);
			//cb();
		}
